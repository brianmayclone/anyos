...
fn mir_binary_op() {
    ...
    let has_binop = mir.basic_blocks.iter().any(|bb| {
        bb.statements
            .iter()
            .any(|s| matches!(&s.kind, StatementKind::Assign(_, Rvalue::BinaryOp(..))))
    });
    assert!(has_binop);
}
...
fn mir_if_else_creates_branches() {
    ...
    let has_switch = mir
        .basic_blocks
        .iter()
        .any(|bb| matches!(&bb.terminator, Terminator::SwitchInt { .. }));
    assert!(has_switch);
}
...
fn mir_loop() {
    let mir =
        build_fn_mir("fn foo() { let mut i: i32 = 0; loop { i = i + 1; if i > 10 { break; } } }");
    let has_back_edge = mir
        .basic_blocks
        .iter()
        .enumerate()
        .any(|(idx, bb)| match &bb.terminator {
...
fn mir_struct_construction() {
    let bodies = build_mir(
        r#"
        struct Point { x: i32, y: i32 }
        fn main() { let p = Point { x: 1, y: 2 }; }
    "#,
    );
    let main_mir = bodies.iter().find(|b| b.locals.len() > 0).unwrap();
    let has_aggregate = main_mir.basic_blocks.iter().any(|bb| {
        bb.statements
            .iter()
            .any(|s| matches!(&s.kind, StatementKind::Assign(_, Rvalue::Aggregate(..))))
    });
