...
/// Apps can pass this to their shader as a uniform for shadow projection.
/// Returns null if no shadow map is active.
#[no_mangle]
pub extern "C" fn gl_shadow_get_light_mvp() -> *const f32 {
    let c = ctx();
    if c.shadow_map_ready || c.shadow_pass_active {
        c.shadow_light_mvp.as_ptr()
    } else {
        core::ptr::null()
    }
}

/// Query whether a shadow map is available for the current frame.
/// Returns 1 if shadow pass was completed, 0 otherwise.
#[no_mangle]
pub extern "C" fn gl_shadow_available() -> u32 {
    if ctx().shadow_map_ready { 1 } else { 0 }
}

/// Get the shadow map texture unit (always 7).
/// Apps use this with glUniform1i(shadowSamplerLoc, gl_shadow_get_unit()).
#[no_mangle]
