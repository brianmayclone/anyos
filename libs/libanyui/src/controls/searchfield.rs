...
impl Control for SearchField {
    ...
    fn render(&self, surface: &crate::draw::Surface, ax: i32, ay: i32) {
        ...
        let palette =
            crate::controls::chrome::flat_field_palette(0, ctx.hovered, self.focused, ctx.disabled);
        ...
        if self.text_base.text.is_empty() {
            ...
            if self.has_selection() && self.focused {
                ...
                let text = &self.text_base.text;
                let start_px =
                    crate::draw::text_width_n_at(text, sel_start.min(text.len()), font_size) as i32;
                let end_px =
                    crate::draw::text_width_n_at(text, sel_end.min(text.len()), font_size) as i32;
