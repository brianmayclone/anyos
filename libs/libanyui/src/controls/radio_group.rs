...
use crate::control::{find_idx, ChildLayout, Control, ControlBase, ControlId, ControlKind};
use alloc::boxed::Box;
use alloc::vec::Vec;
...
pub(crate) fn post_deselect(group_id: ControlId, selected_id: ControlId) {
    unsafe {
        let pending = core::ptr::addr_of_mut!(PENDING);
        let pending_count = core::ptr::addr_of_mut!(PENDING_COUNT);
...
pub(crate) fn drain_deselects(controls: &mut [Box<dyn Control>]) -> Vec<ControlId> {
    let count = unsafe { core::ptr::addr_of!(PENDING_COUNT).read() };
    if count == 0 {
        return Vec::new();
    }
    ...
    for i in 0..count {
        let (group_id, selected_id) = unsafe { core::ptr::addr_of!(PENDING).read()[i] };
    ...
    unsafe { core::ptr::addr_of_mut!(PENDING_COUNT).write(0) };
    affected_groups
}
