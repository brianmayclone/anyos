...
impl ContextMenu {
    pub fn new(text_base: TextControlBase) -> Self {
        let mut cm = Self { text_base, hovered_item: u32::MAX, has_icons: false, icon_cache: alloc::vec::Vec::new() };
    ...
    fn recompute_size(&mut self) {
        ...
        let icon_extra = if self.has_icons { ICON_SZ as u32 + 8 } else { 0 };
    ...
    fn render(&self, surface: &crate::draw::Surface, ax: i32, ay: i32) {
        ...
        for (i, item_text) in items.iter().enumerate() {
            if is_divider(item_text) {
                ...
                let icon_offset = if self.has_icons { crate::theme::scale_i32(ICON_SZ + 8) } else { 0 };
                ...
                if i < self.icon_cache.len() {
                    ...
                    if !pixels.is_empty() && iw > 0 && ih > 0 {
                        ...
                        crate::draw::blit_buffer_scaled(surface, icon_x, icon_y, icon_sz, icon_sz, iw, ih, pixels);
                    }
                }
                ...
                if !label.is_empty() {
                    ...
                    crate::draw::draw_text_sized(surface, x + text_pad_x + icon_offset, iy + text_pad_y, text_color, label, fs);
                }
                iy += item_h;
            }
        }
