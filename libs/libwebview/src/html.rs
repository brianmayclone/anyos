...
pub fn tokenize(html: &str) -> Vec<Token> {
    ...
    while pos < bytes.len() {
        if bytes[pos] == b'<' {
            ...
            let is_raw = name == "script" || name == "style" || name == "svg" || name == "noscript";
...
pub fn parse(html: &str) -> Dom {
    ...
    for tok in tokens {
        match tok {
            ...
            Token::StartTag {
                ...
                let mut dom_attrs: Vec<Attr> = attrs
                    .into_iter()
                    .map(|(n, v)| Attr { name: n, value: v })
                    .collect();

                // For unknown/custom elements (e.g. <a-analytics>), store the raw
                // lowercase tag name in a synthetic attribute "\x00" so that CSS
                // selector matching can distinguish between different custom elements.
                if tag == Tag::Unknown {
                    dom_attrs.push(Attr {
                ...
                match tag {
                    Tag::Html => {
                        ...
                        if let NodeType::Element { ref mut attrs, .. } = dom.nodes[root].node_type {
                            *attrs = dom_attrs;
                        }
                        continue;
                    }
    ...
    #[cfg(feature = "debug_surf")]
    {
        let max_depth = dom
            ...
            .unwrap_or(0);
        crate::debug_surf!(
            "[html] tree build done: {} nodes, max_depth={}",
            dom.nodes.len(),
            max_depth
        );
        crate::debug_surf!(
        ...
        match tok {
            ...
            Token::StartTag {
                ...
                if tag == Tag::Td || tag == Tag::Th {
                    if let Some(&top) = stack.last() {
                        let t = node_tag(&dom, top);
                        if t == Some(Tag::Td) || t == Some(Tag::Th) {
                            stack.pop();
                        }
                    }
                }
