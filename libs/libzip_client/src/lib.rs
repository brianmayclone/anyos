...
impl ZipReader {
    ...
    pub fn extract_to_file(&self, index: u32, path: &str) -> bool {
        (lib().libzip_extract_to_file)(self.handle, index, path.as_ptr(), path.len() as u32) == 0
    }
}
...
impl ZipWriter {
    ...
    pub fn write_to_file(self, path: &str) -> bool {
        let result = (lib().libzip_write_to_file)(self.handle, path.as_ptr(), path.len() as u32) == 0;
...
impl TarReader {
    ...
    /// Extract an entry directly to a file.
    pub fn extract_to_file(&self, index: u32, path: &str) -> bool {
        (lib().libzip_tar_extract_to_file)(self.handle, index, path.as_ptr(), path.len() as u32) == 0
    }
}
