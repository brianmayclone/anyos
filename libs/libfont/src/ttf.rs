...
impl TtfFont {
    ...
    pub fn char_to_glyph(&self, codepoint: u32) -> u16 {
        ...
        if cp < start_code || cp > end_code { return 0; }
        ...
        if glyph_id_addr + 2 > d.len() { return 0; }
    ...
    fn parse_simple_glyph(
        &self, abs: usize, num_contours: i16,
        ...
        let num_points = match contour_ends.last() {
            ...
            None => {
                return Some(GlyphOutline {
                    ...
                    contour_ends, points: Vec::new(),
                });
            }
        };

        if d.len() < off + 2 { return None; }
        ...
        for i in 0..num_points {
            ...
            if f & X_SHORT_VECTOR != 0 {
                ...
                let dx = d[off] as i16;
                off += 1;
                if f & X_IS_SAME_OR_POSITIVE != 0 { x += dx; } else { x -= dx; }
        ...
        for i in 0..num_points {
            ...
            if f & Y_SHORT_VECTOR != 0 {
                ...
                let dy = d[off] as i16;
                off += 1;
                if f & Y_IS_SAME_OR_POSITIVE != 0 { y += dy; } else { y -= dy; }
        ...
        Some(GlyphOutline {
            num_contours, x_min, y_min, x_max, y_max, contour_ends, points,
        })
    }
        ...
        loop {
            ...
            if let Some(component) = self.glyph_outline_depth(component_glyph, depth + 1) {
                ...
                for pt in &component.points {
                    ...
                    let tx = if flags & ARGS_ARE_XY_VALUES != 0 { arg1 } else { 0 };
                    let ty = if flags & ARGS_ARE_XY_VALUES != 0 { arg2 } else { 0 };

                    let new_x = ((a * px + b * py) >> 14) + tx;
                    let new_y = ((c * px + dd * py) >> 14) + ty;

                    all_points.push(GlyphPoint {
                        x: new_x as i16, y: new_y as i16, on_curve: pt.on_curve,
                    });
                }
