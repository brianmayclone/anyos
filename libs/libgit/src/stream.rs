...
impl HttpStream {
    ...
    pub fn post(
        ...
        let chunked = header_str.to_lowercase().contains("transfer-encoding: chunked");
    ...
    fn read_next_chunk_header(&mut self) {
        ...
        loop {
            ...
            if let Some(crlf) = remaining.windows(2).position(|w| w == b"\r\n") {
                ...
                anyos_std::println!("[stream] chunk: size_str='{}' → {} bytes", size_str.trim(), self.chunk_remaining);
                self.buf_pos += crlf + 2;
...
fn tls_recv_patient(handle: u32, sock: u32, buf: &mut [u8]) -> i32 {
    for _ in 0..5 {
        ...
        if avail == u32::MAX || avail == u32::MAX - 1 { return 0; }
...
fn parse_http_status(headers: &str) -> u16 {
    ...
    if let Some(line) = headers.lines().next() {
        if let Some(rest) = line.strip_prefix("HTTP/1.1 ").or_else(|| line.strip_prefix("HTTP/1.0 ")) {
...
fn tls_recv_cb(fd: u32, buf: &mut [u8]) -> i32 {
    ...
    match avail {
        ...
        _ => {
            ...
            let n = anyos_std::net::tcp_recv(fd, buf);
            if n == 0 { 0 } else if n != u32::MAX { n as i32 } else { -1 }
        }
    }
}

fn tls_sleep_cb(ms: u32) { anyos_std::process::sleep(ms); }
fn tls_random_cb(buf: &mut [u8]) -> u32 { anyos_std::sys::random(buf) }
