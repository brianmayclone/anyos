...
pub fn math_expm1(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    if x == 0.0 {
        return JsValue::Number(x);
    }
    if x.is_infinite() {
        return JsValue::Number(if x > 0.0 { f64::INFINITY } else { -1.0 });
    }
...
pub fn math_asin(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    if x == 1.0 {
        return JsValue::Number(core::f64::consts::FRAC_PI_2);
    }
    if x == -1.0 {
        return JsValue::Number(-core::f64::consts::FRAC_PI_2);
    }
...
pub fn math_acos(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    if x == -1.0 {
        return JsValue::Number(core::f64::consts::PI);
    }
...
pub fn math_atan(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    if x.is_infinite() {
        return JsValue::Number(if x > 0.0 {
...
pub fn math_tanh(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    if x.is_infinite() {
        return JsValue::Number(if x > 0.0 { 1.0 } else { -1.0 });
    }
    // Preserve -0: tanh(-0) = -0
    if x == 0.0 {
        return JsValue::Number(x);
...
pub fn floor_f64(n: f64) -> f64 {
    ...
    if n >= I64_MAX_F64 || n <= -I64_MAX_F64 {
        return n;
    }
    let i = n as i64;
    if (i as f64) <= n {
