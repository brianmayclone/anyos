...
use crate::value::*;
use super::Vm;
use super::native_math::{floor_f64, trunc_f64, ln_approx};
...
pub fn number_to_precision(vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    let prec = args.first().map(|v| v.to_number() as usize).unwrap_or(1).max(1).min(100);

    if n.is_nan() { return JsValue::String(String::from("NaN")); }
    if n.is_infinite() {
        return JsValue::String(if n > 0.0 { String::from("Infinity") } else { String::from("-Infinity") });
    }
    ...
    if abs == 0.0 {
        ...
        if prec > 1 {
            s.push('.');
            for _ in 0..prec - 1 { s.push('0'); }
        }
        if negative { let mut r = String::from("-"); r.push_str(&s); return JsValue::String(r); }
    ...
    if e >= 0 && (e as usize) < prec {
        ...
        for (i, ch) in digits_str.chars().enumerate() {
            if i == int_digits && i < digits_str.len() { result.push('.'); }
            result.push(ch);
        }
        // Pad with zeros if needed
        while result.replace('-', "").replace('.', "").len() < prec {
            if !result.contains('.') { result.push('.'); }
            result.push('0');
        }
    } else {
        // Exponential notation
        let mut chars = digits_str.chars();
        if let Some(first) = chars.next() { result.push(first); }
...
pub fn number_to_exponential(vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    let frac_digits = if args.is_empty() || matches!(args.first(), Some(JsValue::Undefined)) {
        ...
        Some(args.first().map(|v| v.to_number() as usize).unwrap_or(0).min(100))
    };
    ...
    let prec = frac_digits.unwrap_or_else(|| {
        ...
        while p < 20 {
            ...
            if (shifted * factor - abs).abs() < 1e-10 { break; }
            p += 1;
        }
        p
    });
...
pub fn number_is_safe_integer(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    match args.first() {
        Some(JsValue::Number(n)) => {
            JsValue::Bool(!n.is_nan() && !n.is_infinite() && *n == trunc_f64(*n) && n.abs() <= 9007199254740991.0)
        }
