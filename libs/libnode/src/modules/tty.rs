...
pub fn module() -> JsValue {
    ...
    module.set(String::from("WriteStream"), native_fn("WriteStream", stream));
    object(module)
}

fn isatty(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    let fd = args.first().map(|value| value.to_number() as i32).unwrap_or(-1);
...
fn stream(_vm: &mut Vm, args: &[JsValue]) -> JsValue {
    ...
    stream.set_property(String::from("isTTY"), JsValue::Bool(matches!(fd as i32, 0..=2)));
