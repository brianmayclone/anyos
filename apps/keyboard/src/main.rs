...
impl KeyDef {
    ...
    const fn modkey(sc: u8, w: i32) -> Self {
        KeyDef {
            scancode: sc,
            width: w,
            is_mod: true,
...
fn main() {
    ...
    let mut layouts_buf = [kbd::LayoutInfo {
        id: 0,
        code: [0; 8],
        label: [0; 4],
    }; 8];
    ...
    for &row in ROWS {
        ...
        for kd in row {
            ...
            if !label.is_empty() {
                ...
                a.canvas
                    .draw_text(text_x, text_y, text_color, 0, 13, &label);
            }
            ...
            if !kd.is_mod && kd.scancode != 0x39 {
                ...
                if !alt_label.is_empty() && alt_label != label {
                    a.canvas
                        .draw_text(x + 4, y + 3, KEY_TEXT_DIM, 0, 10, &alt_label);
                }
            }
