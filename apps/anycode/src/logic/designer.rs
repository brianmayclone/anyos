    ...
    match target {
        ...
        UiCodeTarget::Node => {
            ...
            if !crate::util::path::exists(&events_path) {
                ...
                ensure_event_stubs_js(&events_path, doc)?;
            }
            anyos_std::fs::write_bytes(
...
fn ensure_event_stubs_js(events_path: &str, doc: &DesignerDocument) -> Result<(), &'static str> {
    let mut data = anyos_std::fs::read_to_string(events_path).unwrap_or_default();
    let mut changed = false;
    for control in &doc.controls {
        for (_event_name, handler) in event_handler_bindings(control) {
            let signature = format!("function {}()", handler);
            if !data.contains(&signature) {
                if !data.ends_with('\n') {
                    data.push('\n');
                }
            ...
            let export_line = format!("module.exports.{} = {};", handler, handler);
