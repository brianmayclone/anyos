...
impl TcpStream {
    ...
    pub fn upgrade_to_tls(&mut self) -> Result<(), ConnError> {
        let rc = tls::connect(self.socket_id, &self.host);
        if rc != 0 {
            return Err(ConnError::TlsHandshakeFailed);
        }
        self.use_tls = true;
    ...
    pub fn send(&self, data: &[u8]) -> Result<usize, ConnError> {
        if self.use_tls {
            let n = tls::send(data);
            if n < 0 { return Err(ConnError::SendFailed); }
            Ok(n as usize)
        } else {
            let n = net::tcp_send(self.socket_id, data);
            if n == u32::MAX { return Err(ConnError::SendFailed); }
            Ok(n as usize)
        }
    }
    ...
    pub fn recv(&self, buf: &mut [u8]) -> Result<usize, ConnError> {
        if self.use_tls {
            let n = tls::recv(buf);
            if n < 0 { return Err(ConnError::RecvFailed); }
            if n == 0 { return Err(ConnError::Closed); }
            Ok(n as usize)
        } else {
            let n = net::tcp_recv(self.socket_id, buf);
            if n == u32::MAX { return Err(ConnError::RecvFailed); }
            if n == 0 { return Err(ConnError::Closed); }
            Ok(n as usize)
        }
    }
    ...
    pub fn recv_line(&mut self) -> Result<String, ConnError> {
        ...
        loop {
            ...
            let n = if self.use_tls {
                let r = tls::recv(&mut self.read_buf);
                if r < 0 { return Err(ConnError::RecvFailed); }
                r as usize
            } else {
                ...
                loop {
                    ...
                    retries += 1;
                    if retries > 10000 { return Err(ConnError::Timeout); }
                }
                if n == u32::MAX { return Err(ConnError::RecvFailed); }
                n as usize
            };
    ...
    pub fn recv_exact(&mut self, count: usize) -> Result<Vec<u8>, ConnError> {
        ...
        while remaining > 0 {
            ...
            let n = if self.use_tls {
                let r = tls::recv(&mut buf[..to_read]);
                if r < 0 { return Err(ConnError::RecvFailed); }
                r as usize
            } else {
                let r = net::tcp_recv(self.socket_id, &mut buf[..to_read]);
                if r == u32::MAX { return Err(ConnError::RecvFailed); }
                r as usize
            };
