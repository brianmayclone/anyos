...
fn bench_file_read_large(state: &mut BenchState, hz: u32) {
    ...
    let per = if iters > 0 { us / iters } else { 0 };
    log!(
        "  {} iterations, {} KiB total, {} us ({} us/read)",
        iters,
        total_bytes / 1024,
        us,
        per
...
fn stress_iteration(_hz: u32) -> (u32, u32) {
    ...
    // 8. Window IPC: create + fill + present + destroy
    {
        ...
        calls += 1;
        if wid != u32::MAX {
            window::fill_rect(wid, 0, 0, 64, 64, 0xFF2D2D2D);
            calls += 1;
            window::draw_text(wid, 2, 2, 0xFFFFFFFF, "S");
            calls += 1;
            window::present(wid);
            calls += 1;
            window::destroy(wid);
...
fn update_ui() {
    ...
    match a.mode {
        Mode::Idle => {
            ...
            a.lbl_line2
                .set_text("or Stress Test for a continuous loop.");
        ...
        Mode::Bench => {
            ...
            let pct = if total > 0 {
                (cur as u32 * 100 / total as u32).min(100)
            } else {
                0
            };
            let mut buf = [0u8; 40];
        ...
        Mode::Stress => {
            ...
            let status_color = if a.stress.errors == 0 {
                0xFF00FF80
            } else {
                0xFFFF4040
            };
            let status_text = if a.stress.errors == 0 {
                "RUNNING - all OK"
            } else {
                "RUNNING - ERRORS"
            };
            ...
            let mut buf1 = [0u8; 32];
            a.lbl_stat1
                .set_text(fmt_kv(&mut buf1, "Iterations: ", a.stress.iterations));

            let mut buf2 = [0u8; 32];
            a.lbl_stat2.set_text(fmt_kv(
                &mut buf2,
                "Syscalls:   ",
                a.stress.total_syscalls as u32,
            ));

            let mut buf3 = [0u8; 32];
            a.lbl_stat3
                .set_text(fmt_elapsed_label(&mut buf3, "Elapsed:    ", elapsed));

            let mut buf4 = [0u8; 32];
            a.lbl_stat4
                .set_text(fmt_kv(&mut buf4, "Errors:     ", a.stress.errors));
            a.lbl_stat4.set_text_color(if a.stress.errors == 0 {
                0xFF00FF80
            } else {
                0xFFFF4040
            });
        ...
        Mode::StressDone => {
            ...
            let mut buf3 = [0u8; 32];
            a.lbl_stat3.set_text(fmt_elapsed_label(
                &mut buf3,
                "Duration:   ",
                a.stress.elapsed_sec,
            ));
...
fn main() {
    ...
    grid.set_columns(&[
        ...
        anyui::ColumnDef::new(i18n::t("Time"))
            .width(100)
            .align(anyui::ALIGN_RIGHT),
        anyui::ColumnDef::new(i18n::t("Iters"))
            .width(80)
            .align(anyui::ALIGN_RIGHT)
            .numeric(),
    ]);
    ...
    app().btn_start.on_click(|_| {
        ...
        match a.mode {
            ...
            Mode::Stress => {
                ...
                log!(
                    "  STRESS TEST COMPLETE: {} iters, {} errors, {}s",
                    a.stress.iterations,
                    a.stress.errors,
                    elapsed
                );
                ...
                if a.bench.current_test < a.bench.total_tests {
                    ...
                    for i in 0..a.bench.count {
                        if let Some(ref r) = a.bench.results[i] {
                            let name =
                                unsafe { core::str::from_utf8_unchecked(&r.name[..r.name_len]) };
                            log!(
                                "  {:<24} {:>8} us  ({} iters)",
                                name,
                                r.value_us,
