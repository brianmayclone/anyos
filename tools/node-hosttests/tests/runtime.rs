...
#[test]
fn anyos_anyui_core_alias_is_not_shadowed_by_installed_package() {
    let root = temp_project("anyos-anyui-core-alias");
    let package_dir = root.join("node_modules").join("@anyos").join("anyui");
    fs::create_dir_all(&package_dir).expect("failed to create package dir");
    fs::write(
        package_dir.join("package.json"),
        br#"{"name":"@anyos/anyui","version":"0.1.0","main":"index.js"}"#,
    )
    .expect("failed to write package.json");
    fs::write(
        package_dir.join("index.js"),
        b"module.exports = { run: 'wrong package shadow' };",
    )
    .expect("failed to write shadowing index.js");
...
#[test]
fn anycode_standard_generated_node_ui_project_runs_like_anycode_executes_it() {
...
MainForm.prototype.root = function() {
    return this.ui.root;
};

module.exports = MainForm;
module.exports.MainForm = MainForm;
module.exports.MainFormUi = designer.MainFormUi;
"#,
    )
    .expect("failed to write view.js");
    fs::write(
        &form_dir.join("index.js"),
        r#"const view = require('./view');
const designer = require('./designer');

module.exports = view;
module.exports.MainForm = view.MainForm || view;
module.exports.MainFormUi = designer.MainFormUi;
"#,
    )
    ...
    let mut runtime = runtime_for(&root);
    let export_probe = runtime.run_script(
        &root.join("src/export_probe.js").to_string_lossy(),
        "\
        const formModule = require('./ui/main_form'); \
        [typeof formModule, typeof formModule.MainForm, typeof formModule.MainFormUi, typeof formModule.MainFormUi.build].join(':');",
    );
    assert_eq!(
        export_probe.to_js_string(),
...
"#,
    ...
    fs::write(
        root.join(".anycode-workspace"),
        format!(
            "anycode-workspace-v1\nstartup_project=anycode-standard-ui-app\nstartup_form=\nstartup_storyboard={}\nstartup_run_config=\nbuild_order=anycode-standard-ui-app\nunloaded_projects=\n",
...
const formModule = require('./ui/main_form');
const viewModule = require('./ui/main_form/view');
const designerModule = require('./ui/main_form/designer');

const STARTUP_STORYBOARD = "{}";

function GeneratedAnyCodeForm(uiBuilder) {{
...
function createForm() {{
  if (viewModule && typeof viewModule.MainForm === 'function') {{
    return new viewModule.MainForm();
  }}
  if (typeof viewModule === 'function') {{
    return new viewModule();
  }}
  if (formModule && typeof formModule.MainForm === 'function') {{
    return new formModule.MainForm();
  }}
  if (typeof formModule === 'function') {{
    return new formModule();
  }}
  if (designerModule && designerModule.MainFormUi && typeof designerModule.MainFormUi.build === 'function') {{
    return new GeneratedAnyCodeForm(designerModule.MainFormUi);
  }}
  if (formModule && formModule.MainFormUi && typeof formModule.MainFormUi.build === 'function') {{
    return new GeneratedAnyCodeForm(formModule.MainFormUi);
  }}
  throw new TypeError("Generated form 'MainForm' did not export a constructor");
}}
...
"#,
    ...
    for relative in [
        ...
        ".eslintrc.json",
        ".anycode-workspace",
        "src/types/anyos-anyui.d.ts",
        "src/ui/Main.Storyboard",
        "src/ui/Main.Storyboard.Designer.js",
        "src/ui/Main.Storyboard.codebehind.js",
        "src/ui/main_form/MainForm.Designer",
        "src/ui/main_form/designer.js",
        "src/ui/main_form/events.js",
        "src/ui/main_form/view.js",
        "src/ui/main_form/index.js",
        "src/main.js",
    ] {
        assert!(
            root.join(relative).is_file(),
            "standard anyCode project is missing generated file {}",
            relative
        );
    }

    let mut runtime = runtime_for(&root);
    let storyboard_probe = runtime.run_script(
        &root.join("src/storyboard_probe.js").to_string_lossy(),
        "\
        const story = require('./ui/Main.Storyboard.Designer'); \
        const hooks = require('./ui/Main.Storyboard.codebehind'); \
        story.STORYBOARD_NAME + ':' + story.SCENES.length + ':' + story.SEGUES.length + ':' + typeof hooks.storyboardNavigate",
    );
    assert_eq!(storyboard_probe.to_js_string(), "Main:1:0:function");
    ...
    let npm_run_output = run_npm(&["run", "start"], &root);
    assert!(
        npm_run_output.status.success(),
        "generated anyCode Node UI app failed through npm run start\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&npm_run_output.stdout),
        String::from_utf8_lossy(&npm_run_output.stderr)
    );
}
