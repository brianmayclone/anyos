    ...
    if let Some(font) = parse_ccf(CCF_DATA) {
        ...
        *guard = Some(font);
        crate::serial_verbose_println!(
            "[OK] Cape Coral font initialized ({} sizes)",
...
fn find_size<'a>(sizes: &'a [FontSizeData], target: u16) -> Option<&'a FontSizeData> {
    ...
    sizes
        .iter()
        .min_by_key(|s| (s.size as i32 - target as i32).unsigned_abs())
}
...
fn parse_ccf(data: &[u8]) -> Option<CcFont> {
    ...
    for _ in 0..num_sizes {
        ...
        if offset + glyph_table_size > data.len() {
            return None;
        }
        ...
        for g in &glyphs {
            let end =
                g.bitmap_offset as usize + (g.bitmap_width as usize * g.bitmap_height as usize);
