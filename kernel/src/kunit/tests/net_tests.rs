...
pub static SUITE: TestSuite = TestSuite {
    ...
    cases: &[
        TestCase {
            name: "empty_slice",
            run: test_empty_slice,
        },
        TestCase {
            name: "all_zeros_even",
            run: test_all_zeros_even,
        },
        TestCase {
            name: "all_zeros_odd",
            run: test_all_zeros_odd,
        },
        TestCase {
            name: "all_ones",
            run: test_all_ones,
        ...
        TestCase {
            name: "rfc1071_example",
            run: test_rfc1071_example,
        },
        TestCase {
            name: "known_ip_header",
            run: test_known_ip_header,
        },
        TestCase {
            name: "checksum_of_checksummed",
            run: test_checksum_of_checksummed,
        },
        TestCase {
            name: "odd_length_bytes",
            run: test_odd_length_bytes,
        },
        TestCase {
            name: "pseudo_header_fields",
            run: test_pseudo_header_fields,
        },
        TestCase {
            name: "pseudo_header_tcp_protocol",
            run: test_pseudo_header_tcp_protocol,
        },
    ],
};
...
fn test_all_zeros_even(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(
        internet_checksum(&[0u8; 20]),
        0xFFFFu16,
        "20 zero bytes → 0xFFFF",
    );
}
...
fn test_all_zeros_odd(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(
        internet_checksum(&[0u8; 21]),
        0xFFFFu16,
        "21 zero bytes → 0xFFFF",
    );
}
...
fn test_two_bytes(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(
        internet_checksum(&[0x00u8, 0x01u8]),
        0xFFFEu16,
        "[0x00, 0x01] → 0xFFFE",
    );
}
...
fn test_known_ip_header(ctx: &mut TestContext) {
    ...
    ctx.expect_ne(
        cksum,
        0xFFFFu16,
        "computed checksum is not 0xFFFF (non-trivial header)",
    );
}
...
fn test_odd_length_bytes(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(
        internet_checksum(&[0x01u8]),
        0xFEFFu16,
        "single byte 0x01 → 0xFEFF",
    );
    // [0x01, 0x02, 0x03]: sum = 0x0102 + 0x0300 = 0x0402, complement = 0xFBFD.
    ctx.expect_eq(
        internet_checksum(&[0x01u8, 0x02, 0x03]),
        0xFBFDu16,
        "[0x01,0x02,0x03] → 0xFBFD",
    );
}
