...
pub static SUITE: TestSuite = TestSuite {
    ...
    cases: &[
        ...
        TestCase {
            name: "rfc1321_message_digest",
            run: test_message_digest,
        },
        TestCase {
            name: "rfc1321_a_to_z",
            run: test_a_to_z,
        ...
        TestCase {
            name: "rfc1321_alphanumeric",
            run: test_alphanumeric,
        },
        TestCase {
            name: "rfc1321_8x_digits",
            run: test_8x_digits,
        },
        TestCase {
            name: "md5_hex_format",
        ...
        TestCase {
            name: "different_inputs_differ",
            run: test_differs,
        },
        TestCase {
            name: "same_input_stable",
            run: test_stable,
        },
        TestCase {
        ...
        TestCase {
            name: "sha256_hex_format",
            run: test_sha256_hex_format,
        },
        TestCase {
            name: "password_hash_roundtrip",
            run: test_password_roundtrip,
        },
        TestCase {
            name: "password_rehash_legacy_md5",
            run: test_password_needs_rehash,
        },
    ],
};
...
fn test_hex_format(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(
        hex,
        expected,
        "md5_hex produces correct lowercase hex for \"abc\"",
    );
...
fn test_password_roundtrip(ctx: &mut TestContext) {
    ...
    ctx.expect_true(
        encoded.starts_with("pbkdf2-sha256$"),
        "password hash uses PBKDF2-SHA256 format",
    );
    ctx.expect_true(
        verify_password("s3cret", &encoded),
        "password hash verifies with original password",
    );
    ctx.expect_true(
