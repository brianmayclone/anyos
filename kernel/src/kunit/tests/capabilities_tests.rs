...
use crate::task::capabilities::{
    parse_capabilities, required_cap,
    CAP_ALL, CAP_AUDIO, CAP_AUTO_GRANTED, CAP_COMPOSITOR, CAP_DEBUG,
    CAP_DEFAULT, CAP_DEVICE, CAP_DLL, CAP_EVENT, CAP_FILESYSTEM,
    CAP_MANAGE_PERMS, CAP_NETWORK, CAP_PIPE, CAP_PROCESS, CAP_SHM,
    CAP_SYSTEM, CAP_THREAD,
};

pub static SUITE: TestSuite = TestSuite {
    name: "task::capabilities",
    cases: &[
        TestCase { name: "parse_single_cap",          run: test_parse_single },
        TestCase { name: "parse_multiple_caps",       run: test_parse_multiple },
        TestCase { name: "parse_unknown_ignored",     run: test_parse_unknown },
        TestCase { name: "parse_empty_string",        run: test_parse_empty },
        TestCase { name: "cap_all_has_all_bits",      run: test_cap_all },
        TestCase { name: "cap_default_subset",        run: test_cap_default },
        TestCase { name: "cap_auto_granted_subset",   run: test_cap_auto_granted },
        TestCase { name: "required_cap_basic_syscalls_zero", run: test_basic_syscalls },
        TestCase { name: "required_cap_fs_always_allowed",  run: test_fs_always_allowed },
        TestCase { name: "required_cap_network",      run: test_net_syscalls },
        TestCase { name: "required_cap_audio",        run: test_audio_syscalls },
        TestCase { name: "required_cap_display",      run: test_display_syscalls },
        TestCase { name: "cap_check_logic",           run: test_cap_check_logic },
    ],
};
...
fn test_parse_single(ctx: &mut TestContext) {
    ctx.expect_eq(parse_capabilities("filesystem"), CAP_FILESYSTEM, "filesystem");
    ...
    ctx.expect_eq(parse_capabilities("compositor"), CAP_COMPOSITOR, "compositor");
    ...
    ctx.expect_eq(parse_capabilities("manage_perms"), CAP_MANAGE_PERMS, "manage_perms");
...
fn test_parse_multiple(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(caps, CAP_FILESYSTEM | CAP_NETWORK | CAP_AUDIO, "filesystem+network+audio");
    ...
    ctx.expect_eq(parse_capabilities(all_str), CAP_ALL, "all caps string == CAP_ALL");
}
...
fn test_parse_unknown(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(parse_capabilities("filesystem,bogus"),   CAP_FILESYSTEM, "unknown ignored");
...
fn test_cap_all(ctx: &mut TestContext) {
    ...
    for (name, cap) in &[
        ("filesystem", CAP_FILESYSTEM), ("network", CAP_NETWORK),
        ("audio", CAP_AUDIO), ("device", CAP_DEVICE), ("process", CAP_PROCESS),
        ("pipe", CAP_PIPE), ("shm", CAP_SHM), ("event", CAP_EVENT),
        ("compositor", CAP_COMPOSITOR), ("system", CAP_SYSTEM),
        ("dll", CAP_DLL), ("thread", CAP_THREAD),
        ("manage_perms", CAP_MANAGE_PERMS), ("debug", CAP_DEBUG),
    ] {
...
fn test_cap_default(ctx: &mut TestContext) {
    ...
    ctx.expect_eq(CAP_DEFAULT & CAP_FILESYSTEM, CAP_FILESYSTEM, "default includes filesystem");
    ctx.expect_eq(CAP_DEFAULT & CAP_PROCESS,    CAP_PROCESS,    "default includes process");
    ...
    ctx.expect_eq(CAP_DEFAULT & CAP_MANAGE_PERMS,0u32, "default excludes manage_perms");
}

fn test_cap_auto_granted(ctx: &mut TestContext) {
    // Auto-granted caps must be a subset of ALL.
    ctx.expect_eq(CAP_AUTO_GRANTED & !CAP_ALL, 0u32, "auto_granted subset of CAP_ALL");
    // DLL + thread should always be auto-granted.
    ctx.expect_eq(CAP_AUTO_GRANTED & CAP_DLL,    CAP_DLL,    "dll auto-granted");
    ctx.expect_eq(CAP_AUTO_GRANTED & CAP_THREAD, CAP_THREAD, "thread auto-granted");
}
...
fn test_fs_always_allowed(ctx: &mut TestContext) {
    ...
    use crate::syscall;
    ctx.expect_eq(required_cap(syscall::SYS_OPEN),    0u32, "SYS_OPEN always allowed");
    ctx.expect_eq(required_cap(syscall::SYS_READ),    0u32, "SYS_READ always allowed");
    ctx.expect_eq(required_cap(syscall::SYS_WRITE),   0u32, "SYS_WRITE always allowed");
    ctx.expect_eq(required_cap(syscall::SYS_CLOSE),   0u32, "SYS_CLOSE always allowed");
    ctx.expect_eq(required_cap(syscall::SYS_READDIR), 0u32, "SYS_READDIR always allowed");
    ctx.expect_eq(required_cap(syscall::SYS_STAT),    0u32, "SYS_STAT always allowed");
}

fn test_net_syscalls(ctx: &mut TestContext) {
    let mut found = false;
    for n in 0..200u32 {
        if required_cap(n) & CAP_NETWORK != 0 { found = true; break; }
    }
...
fn test_audio_syscalls(ctx: &mut TestContext) {
    let mut found = false;
    for n in 0..200u32 {
        if required_cap(n) & CAP_AUDIO != 0 { found = true; break; }
    }
...
fn test_display_syscalls(ctx: &mut TestContext) {
    ...
    let mut found = false;
    for n in 0..200u32 {
        if required_cap(n) & CAP_COMPOSITOR != 0 { found = true; break; }
    }
