...
pub fn init_and_register(pci: &PciDevice) -> bool {
    ...
    unsafe {
        ...
        for _ in 0..10_000 {
            if rd8(mmio_virt, REG_CMD) & CMD_RESET == 0 {
                break;
            }
            core::hint::spin_loop();
        }
        ...
        crate::serial_println!(
            "  RTL8125: MAC {:02X}:{:02X}:{:02X}:{:02X}:{:02X}:{:02X}",
            mac[0],
            mac[1],
            mac[2],
            mac[3],
        ...
        for i in 0..NUM_RX_DESC {
            ...
            (*desc).opts1 =
                RX_OWN | if i == NUM_RX_DESC - 1 { RX_EOR } else { 0 } | RX_BUF_SIZE as u32;
        ...
        wr16(
            mmio_virt,
            0x3C,
            (INT_ROK | INT_TOK | INT_LINK_CHG | INT_RDU) as u16,
        );
...
fn transmit(data: &[u8]) -> bool {
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(data.as_ptr(), rtl.tx_bufs_virt[idx] as *mut u8, data.len());
    }
    desc.opts1 = TX_OWN
        | TX_FS
        | TX_LS
        | if idx == NUM_TX_DESC - 1 { TX_EOR } else { 0 }
        | data.len() as u32;
    rtl.tx_tail = ((idx + 1) % NUM_TX_DESC) as u16;
    unsafe {
        wr8(rtl.mmio_base, REG_TPPOLL, 0x40);
    }
    true
}
        ...
        if len > 0 && len <= RX_BUF_SIZE {
            ...
            unsafe {
                core::ptr::copy_nonoverlapping(
                    rtl.rx_bufs_phys[idx] as *const u8,
                    pkt.as_mut_ptr(),
                    len,
                );
            }
...
fn recv_packet() -> Option<Vec<u8>> {
    RTL8125_STATE.lock().as_mut()?.rx_queue.pop_front()
}
...
impl super::NetworkDriver for Rtl8125Driver {
    ...
    fn link_up(&self) -> bool {
        RTL8125_STATE
            .lock()
            .as_ref()
            .map(|r| unsafe { rd32(r.mmio_base, REG_PHY_STATUS) & PHY_LINK_STATUS != 0 })
            .unwrap_or(false)
    }
}
...
pub fn probe(pci: &PciDevice) -> Option<Box<dyn crate::drivers::hal::Driver>> {
    if init_and_register(pci) {
        super::create_hal_driver("Realtek RTL8125 2.5GbE")
    } else {
        None
    }
}
