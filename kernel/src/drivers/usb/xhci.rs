...
fn send_command(ctrl: &mut XhciController, trb: Trb) -> Result<u32, &'static str> {
    ...
    unsafe { ring_write(ctrl.cmd_ring_phys, idx, trb, ctrl.cmd_cycle); }
    ...
    if ctrl.cmd_enqueue == RING_SIZE - 1 {
        ...
        unsafe { ring_write(ctrl.cmd_ring_phys, RING_SIZE - 1, link, ctrl.cmd_cycle); }
        ctrl.cmd_enqueue = 0;
        ctrl.cmd_cycle ^= 1;
    }
...
fn control_transfer(
    ...
    let status_dir = if data_in || data_len == 0 { TRB_DIR_OUT } else { TRB_DIR_IN };
    ring.push(Trb {
        param:  0,
        status: 0,
...
fn read_data_buf(ctrl: &XhciController, buf: &mut [u8], len: usize) {
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(
            ctrl.data_buf_phys as *const u8,
            buf.as_mut_ptr(),
            to_copy,
        );
    }
...
pub fn bulk_transfer(
    ...
    let dci: u32 = if is_in { ep_num as u32 * 2 + 2 } else { ep_num as u32 * 2 + 1 };
    ...
    for i in 0..num_trbs {
        ...
        let ctrl_field = (TRB_NORMAL << 10)
            | (cycle & TRB_C)
            | if last { TRB_IOC } else { TRB_CH }
            | TRB_ISP;
        unsafe {
        ...
        if enqueue >= RING_SIZE - 1 {
            ...
            unsafe {
                ...
                core::ptr::write_volatile(link_ptr, Trb {
                    param:  ring_phys,
                    status: 0,
        ...
        if let Some(status) = poll_event(ctrl, TRB_TRANSFER_EVENT) {
            ...
            return match cc {
                CC_SUCCESS | CC_SHORT_PACKET => {
                    Ok(total_enqueued.saturating_sub(residual))
                }
...
fn enumerate_device(ctrl: &mut XhciController, port_idx: u8, speed: UsbSpeed) {
    // Reset EP0 ring
    unsafe { core::ptr::write_bytes(ctrl.ep0_ring_phys as *mut u8, 0, 4096); }
    ...
    if let Err(e) = enable_and_address_device(ctrl, port_idx, speed, new_addr) {
        crate::serial_verbose_println!("  xHCI: port {} — enable/address failed: {}", port_idx + 1, e);
        return;
    }
    ...
    match control_transfer(ctrl, 1, &setup, true, 8) {
        Ok(_) => {}
        Err(e) => {
            crate::serial_verbose_println!("  xHCI: port {} — GET_DESCRIPTOR(8) failed: {}", port_idx + 1, e);
            return;
        }
    }
    ...
    let real_mps = if desc_buf[7] > 0 { desc_buf[7] as u16 } else { mps };
    ...
    match control_transfer(ctrl, 1, &setup_full, true, 18) {
        Ok(_) => {}
        Err(e) => {
            crate::serial_verbose_println!("  xHCI: device {} — GET_DESCRIPTOR(18) failed: {}", new_addr, e);
            return;
        }
    }
    read_data_buf(ctrl, &mut desc_buf, 18);
    let dev_desc: DeviceDescriptor = unsafe {
        core::ptr::read_unaligned(desc_buf.as_ptr() as *const _)
    };
    ...
    let total_len = match control_transfer(ctrl, 1, &setup_cfg, true, 9) {
        ...
        Err(e) => {
            crate::serial_verbose_println!("  xHCI: device {} — config hdr failed: {}", new_addr, e);
            return;
        }
    };
    ...
    match control_transfer(ctrl, 1, &setup_cfg_full, true, config_len) {
        Ok(_) => { read_data_buf(ctrl, &mut config_buf, config_len as usize); }
        Err(e) => {
            crate::serial_verbose_println!("  xHCI: device {} — full config failed: {}", new_addr, e);
            return;
        }
    }
    ...
    if let Err(e) = control_transfer(ctrl, 1, &setup_setcfg, false, 0) {
        crate::serial_verbose_println!("  xHCI: device {} — SET_CONFIGURATION failed: {}", new_addr, e);
        return;
    }

    let dev_class    = if dev_desc.b_device_class    != 0 { dev_desc.b_device_class    } else { interfaces.first().map(|i| i.class).unwrap_or(0) };
    let dev_subclass = if dev_desc.b_device_sub_class != 0 { dev_desc.b_device_sub_class } else { interfaces.first().map(|i| i.subclass).unwrap_or(0) };
    let dev_protocol = if dev_desc.b_device_protocol  != 0 { dev_desc.b_device_protocol  } else { interfaces.first().map(|i| i.protocol).unwrap_or(0) };
...
fn reset_port(ctrl: &XhciController, port_idx: u8) -> Option<UsbSpeed> {
    ...
    let portsc = rd32(ctrl.op_base, off);
    wr32(ctrl.op_base, off, (portsc & !(PORTSC_PED | PORTSC_CSC | PORTSC_PEC | PORTSC_WRC | PORTSC_PRC)) | PORTSC_PR);
    delay_ms(50);
    ...
    if portsc & PORTSC_PED == 0 {
        crate::serial_verbose_println!("  xHCI: port {} — reset failed (not enabled)", port_idx + 1);
        return None;
    }
...
fn scan_ports(ctrl: &mut XhciController) {
    for i in 0..ctrl.n_ports {
        ...
        wr32(ctrl.op_base, off, portsc | PORTSC_CSC | PORTSC_PEC | PORTSC_WRC | PORTSC_PRC);
        delay_ms(5);
...
pub fn init_controller(pci: &PciDevice) {
    ...
    let ctx_size  = if hccparams1 & (1 << 2) != 0 { CTX_SIZE_LARGE } else { CTX_SIZE_NORMAL };
    ...
    crate::serial_verbose_println!(
        ...
        hciversion, n_ports, max_slots, ctx_size, dboff, rtsoff,
    );
    ...
    for _ in 0..100 {
        if rd32(op_base, OP_USBSTS) & STS_HCH != 0 { break; }
        delay_ms(1);
    }
    ...
    for _ in 0..200 {
        if rd32(op_base, OP_USBCMD) & CMD_HCRST == 0
        && rd32(op_base, OP_USBSTS) & STS_CNR  == 0 { break; }
        delay_ms(1);
    }
    ...
    let alloc = |label: &str| -> u64 {
        match physical::alloc_contiguous(1) {
            Some(p) => { let addr = p.as_u64(); unsafe { core::ptr::write_bytes(addr as *mut u8, 0, 4096); } addr }
            None    => { crate::serial_verbose_println!("  xHCI: alloc failed: {}", label); 0 }
        }
    };
    ...
    if [dcbaa_phys, cmd_ring_phys, evt_ring_phys, evt_erst_phys,
        input_ctx_phys, device_ctx_phys, ep0_ring_phys, data_buf_phys]
        .iter().any(|&p| p == 0)
    {
...
pub fn poll_ports() {
    let mut guard = XHCI_CTRL.lock();
    let ctrl = match guard.as_mut() { Some(c) => c, None => return };
    ...
    for i in 0..ctrl.n_ports as usize {
        ...
        if connected && !was_connected {
            ...
            crate::serial_verbose_println!("  xHCI: hot-unplug — device removed from port {}", i + 1);
