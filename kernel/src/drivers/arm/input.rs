...
/// Mouse event: relative movement, button change, scroll, or absolute move.
#[derive(Clone, Copy, Default)]
...
static ABS_POS_X: AtomicU32 = AtomicU32::new(0);
static ABS_POS_Y: AtomicU32 = AtomicU32::new(0);
...
struct VirtioInput {
    ...
    num_events: u16,
    /// Device name / classification for logging.
    is_keyboard: bool,
}
...
pub fn init(dev: &VirtioMmioDevice) {
    ...
    // Read device name from config and classify keyboard vs pointer.
    ...
    let is_keyboard = if size > 0 {
        let mut name = [0u8; 64];
        let name_len = (size as usize).min(name.len().saturating_sub(1));
        for (i, slot) in name.iter_mut().enumerate().take(name_len) {
            *slot = dev.read_config_u8(8 + i);
            if slot.is_ascii_uppercase() {
                *slot = slot.to_ascii_lowercase();
            }
        }
        let text = core::str::from_utf8(&name[..name_len]).unwrap_or("");
        text.contains("keyboard") || text.contains("kbd")
    } else {
        false
    };
...
fn process_event(event: &VirtioInputEvent, is_keyboard: bool) {
    match event.type_ {
        EV_KEY => {
            if is_keyboard {
                inject_keyboard_event(event.code, event.value);
            } else {
...
fn inject_keyboard_event(code: u16, value: u32) {
    if value == 2 {
        return;
    }
    let Some((scancode, needs_e0)) = evdev_key_to_scancode(code) else {
        return;
    };
    if needs_e0 {
