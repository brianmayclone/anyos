...
fn wait_cmd_ready(mmio: u64) -> bool {
    for _ in 0..100_000 {
        ...
        if state & STATE_CMD_INHIBIT == 0 { return true; }
        core::hint::spin_loop();
    }
    false
}
...
fn wait_dat_ready(mmio: u64) -> bool {
    for _ in 0..100_000 {
        let state = unsafe { rd32(mmio, SDHCI_PRESENT_STATE) };
        if state & STATE_DAT_INHIBIT == 0 { return true; }
        core::hint::spin_loop();
    }
    false
}
...
fn send_cmd(mmio: u64, cmd: u16, arg: u32, resp_type: u8) -> Option<[u32; 4]> {
    if !wait_cmd_ready(mmio) { return None; }

    // Clear interrupts
    unsafe { wr32(mmio, SDHCI_INT_STATUS, 0xFFFFFFFF); }

    // Set argument
    unsafe { wr32(mmio, SDHCI_ARGUMENT, arg); }

    // Build command register: [13:8]=cmd_index, [1:0]=resp_type, [5]=data_present
    let cmd_reg = ((cmd & 0x3F) << 8) | match resp_type {
        0 => 0x00,   // No response
    ...
    unsafe { wr16(mmio, SDHCI_TRANSFER_MODE + 2, cmd_reg); }

    // Wait for command complete
    for _ in 0..500_000 {
        let status = unsafe { rd32(mmio, SDHCI_INT_STATUS) };
        if status & INT_ERROR != 0 {
            unsafe { wr32(mmio, SDHCI_INT_STATUS, status); }
            return None;
        }
        if status & INT_CMD_COMPLETE != 0 {
            unsafe { wr32(mmio, SDHCI_INT_STATUS, INT_CMD_COMPLETE); }
            let r = unsafe { [
...
fn read_sector_pio(mmio: u64, lba: u32, buf: &mut [u8]) -> bool {
    ...
    if !wait_dat_ready(mmio) { return false; }
    ...
    for _ in 0..500_000 {
        ...
        if status & INT_BUFFER_READ_READY != 0 {
            unsafe { wr32(mmio, SDHCI_INT_STATUS, INT_BUFFER_READ_READY); }
            break;
        }
        core::hint::spin_loop();
    }
    ...
    for _ in 0..100_000 {
        ...
        if status & INT_TRANSFER_COMPLETE != 0 {
            unsafe { wr32(mmio, SDHCI_INT_STATUS, INT_TRANSFER_COMPLETE); }
            return true;
        }
        core::hint::spin_loop();
    }
...
fn write_sector_pio(mmio: u64, lba: u32, buf: &[u8]) -> bool {
    ...
    for _ in 0..500_000 {
        let status = unsafe { rd32(mmio, SDHCI_INT_STATUS) };
        if status & (1 << 4) != 0 { // Buffer Write Ready
            unsafe { wr32(mmio, SDHCI_INT_STATUS, 1 << 4); }
            break;
        }
        core::hint::spin_loop();
    }
    ...
    for i in 0..128 {
        ...
        unsafe { wr32(mmio, SDHCI_BUFFER_DATA, word); }
    }
...
fn sdhci_read(disk_id: u8, lba: u32, count: u32, buf: &mut [u8]) -> bool {
    ...
    let sd = match guard.as_ref() { Some(s) => s, None => return false };
    if disk_id != sd.disk_id { return false; }

    for i in 0..count {
        let offset = (i as usize) * 512;
        if offset + 512 > buf.len() { return false; }
        let addr = if sd.card_sdhc { lba + i } else { (lba + i) * 512 };
...
pub fn init_and_register(pci: &PciDevice) -> bool {
    ...
    // Reset controller
    unsafe { wr8(mmio, SDHCI_SOFTWARE_RESET, RESET_ALL); }
    for _ in 0..100_000 {
        if unsafe { rd8(mmio, SDHCI_SOFTWARE_RESET) } == 0 { break; }
        core::hint::spin_loop();
    }

    // Set timeout
    unsafe { wr8(mmio, SDHCI_TIMEOUT_CONTROL, 0x0E); } // Max timeout

    // Power on: 3.3V
    unsafe { wr8(mmio, SDHCI_POWER_CONTROL, 0x0F); } // SD Bus Power + 3.3V

    // Clock: enable internal clock, wait stable, then enable SD clock
    unsafe {
        wr16(mmio, SDHCI_CLOCK_CONTROL, 0x0001); // Internal Clock Enable
        for _ in 0..10_000 {
            if rd16(mmio, SDHCI_CLOCK_CONTROL) & 0x0002 != 0 { break; } // Internal Clock Stable
            core::hint::spin_loop();
        }
    ...
    if state & STATE_CARD_INSERTED == 0 {
        ...
        *SDHCI_STATE.lock() = Some(SdhciController {
            mmio, irq, card_rca: 0, card_inserted: false,
            card_sdhc: false, total_sectors: 0, disk_id: 10,
        });
    ...
    match init_card(mmio) {
        Some((rca, is_sdhc, sectors)) => {
            ...
            *SDHCI_STATE.lock() = Some(SdhciController {
                mmio, irq, card_rca: rca, card_inserted: true,
                card_sdhc: is_sdhc, total_sectors, disk_id,
            });

            crate::serial_println!("[OK] SDHCI: SD{} card ({} sectors, RCA={:#06x})",
                if is_sdhc { "HC" } else { "SC" }, total_sectors, rca);

            // Emit device attached event
            crate::ipc::event_bus::system_emit(crate::ipc::event_bus::EventData::new(
                crate::ipc::event_bus::EVT_DEVICE_ATTACHED, disk_id as u32, 0, 0, 0,
            ));
        }
        None => {
...
pub fn is_card_inserted() -> bool {
    SDHCI_STATE.lock().as_ref().map(|s| s.card_inserted).unwrap_or(false)
}
