//! Pure Rust SHA-256 implementation (FIPS 180-4) for `no_std`.
...
pub fn sha256(data: &[u8]) -> [u8; SHA256_DIGEST_SIZE] {
    ...
    for i in 0..full_blocks {
        ...
        block.copy_from_slice(&data[i * SHA256_BLOCK_SIZE..(i + 1) * SHA256_BLOCK_SIZE]);
        sha256_compress(&mut state, &block);
    }

    let remainder = &data[full_blocks * SHA256_BLOCK_SIZE..];
    let mut pad_block = [0u8; SHA256_BLOCK_SIZE];
    pad_block[..remainder.len()].copy_from_slice(remainder);
    pad_block[remainder.len()] = 0x80;

    if remainder.len() + 1 > 55 {
        sha256_compress(&mut state, &pad_block);
        pad_block = [0u8; SHA256_BLOCK_SIZE];
    }
...
/// Compute SHA-256 and return a lowercase ASCII hex string.
pub fn sha256_hex(input: &[u8]) -> [u8; SHA256_DIGEST_SIZE * 2] {
    let raw = sha256(input);
    let mut hex = [0u8; SHA256_DIGEST_SIZE * 2];
