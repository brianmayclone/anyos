...
fn main() {
    ...
    if _help_raw.contains("--help") {
        anyos_std::println!("mv - Move or rename files\n\nUsage: mv SOURCE DEST");
        return;
    }
    ...
    if src.is_empty() || dst.is_empty() {
        anyos_std::println!("Usage: mv <source> <destination>");
        return;
    }
    ...
    // Remove source
    if anyos_std::fs::unlink(src) == u32::MAX {
        anyos_std::println!("mv: warning: copied but could not remove '{}'", src);
    }
}
