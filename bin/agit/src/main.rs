...
use alloc::vec::Vec;
use alloc::format;
use libgit::{Repository, Object, ObjectType, Oid, Index, Commit};
use libgit::index::IndexEntry;
use libgit::tree::{TreeEntry, build_tree, parse_tree};
...
fn cmd_init(args: &anyos_std::args::ParsedArgs) {
    ...
    match Repository::init(path) {
        Ok(repo) => {
            anyos_std::println!("Initialized empty Git repository in {}", repo.gitdir.display());
        }
...
fn cmd_clone(args: &anyos_std::args::ParsedArgs) {
    ...
    let url = match libgit::remote::GitUrl::parse(url_str) {
        Some(u) => u,
        None => { anyos_std::println!("fatal: invalid URL: {}", url_str); return; }
    };
    ...
    let wants: Vec<Oid> = remote_refs.iter()
...
fn add_file(repo: &Repository, index: &mut Index, path: &str) {
    ...
    match repo.write_object(&obj) {
        Ok(oid) => {
            let mode = if is_executable(&full_path) { 0o100755 } else { 0o100644 };
    ...
    if let Ok(entries) = std::fs::read_dir(&full_path) {
        for entry in entries {
            if let Ok(entry) = entry {
                ...
                let rel_path = if dir == "." { name.clone() } else { format!("{}/{}", dir, name) };
...
fn cmd_status() {
    ...
    let head_tree = get_head_tree_flat(&repo);
    let gitignore = GitIgnore::load(&repo).with_defaults();
    ...
    if staged_new.is_empty() && staged_modified.is_empty() && staged_deleted.is_empty()
        && modified.is_empty() && deleted.is_empty() && untracked.is_empty()
    {
        anyos_std::println!("\nnothing to commit, working tree clean");
    }
}

fn collect_untracked(repo: &Repository, index: &Index, dir: &str, out: &mut Vec<String>, gitignore: &GitIgnore) {
    let full_path = repo.workdir_path(dir);
    if let Ok(entries) = std::fs::read_dir(&full_path) {
        for entry in entries {
            if let Ok(entry) = entry {
                let name = match entry.file_name().to_str() { Some(n) => String::from(n), None => continue };
                let rel_path = if dir == "." { name } else { format!("{}/{}", dir, name) };
                if let Ok(ft) = entry.file_type() {
                    if gitignore.is_ignored(&rel_path, ft.is_dir()) { continue; }
                    if ft.is_dir() {
...
fn cmd_commit(args: &anyos_std::args::ParsedArgs) {
    ...
    let message = match args.opt(b'm') {
        ...
        None => { anyos_std::println!("error: switch 'm' requires a value"); return; }
    };
    ...
    match repo.current_branch() {
        ...
        _ => { let _ = libgit::refs::detach_head(&repo, &commit_oid); }
    }

    let is_root = commit.parents.is_empty();
    let branch = repo.current_branch().ok().flatten().unwrap_or_else(|| String::from("HEAD"));
    let file_count = index.entries.len();

    anyos_std::println!(
        "[{} {}] {}",
        if is_root { format!("{} (root-commit)", branch) } else { branch },
        commit_oid.short(),
        message,
    );
    anyos_std::println!(" {} file{} changed", file_count, if file_count != 1 { "s" } else { "" });
}
...
fn cmd_log(args: &anyos_std::args::ParsedArgs) {
    ...
    let mut oid = match repo.head() {
        Ok(oid) => oid,
        Err(_) => { anyos_std::println!("fatal: your current branch does not have any commits yet"); return; }
    };

    for _ in 0..max_count {
        let obj = match repo.read_object(&oid) { Ok(o) => o, Err(_) => break };
        if obj.obj_type != ObjectType::Commit { break; }
        let commit = match Commit::parse(&obj.data) { Some(c) => c, None => break };
        ...
        if let Some(parent) = commit.parents.first() { oid = *parent; } else { break; }
    }
}
...
fn cmd_diff() {
    ...
    for entry in &index.entries {
        let full_path = repo.workdir_path(&entry.name);
        let workdir_data = match std::fs::read(&full_path) { Ok(d) => d, Err(_) => continue };
        let workdir_obj = Object::blob(workdir_data.clone());
        if workdir_obj.id() == entry.oid { continue; }

        let index_data = match repo.read_object(&entry.oid) { Ok(o) => o.data, Err(_) => continue };
...
fn cmd_branch(args: &anyos_std::args::ParsedArgs) {
    let repo = match Repository::open(".") {
        Ok(r) => r,
        Err(e) => { anyos_std::println!("fatal: {}", e); return; }
    };
    ...
    if args.pos_count < 2 {
        ...
        if let Ok(branches) = repo.branches() {
            for branch in branches {
                let prefix = if current.as_deref() == Some(&branch) { "\x1b[32m* " } else { "  " };
                let suffix = if current.as_deref() == Some(&branch) { "\x1b[0m" } else { "" };
        ...
        let head_oid = match repo.head() {
            Ok(oid) => oid,
            Err(_) => { anyos_std::println!("fatal: not a valid object name: 'HEAD'"); return; }
        };
...
fn cmd_checkout(args: &anyos_std::args::ParsedArgs) {
    ...
    if let Ok(oid) = libgit::refs::resolve_ref(&repo, &branch_ref) {
        ...
        anyos_std::println!("error: pathspec '{}' did not match any branch or commit", target);
    }
}
...
fn cmd_fetch(args: &anyos_std::args::ParsedArgs) {
    ...
    let remote = match libgit::remote::get_remote(&repo, remote_name) {
        Ok(r) => r,
        Err(_) => { anyos_std::println!("fatal: '{}' does not appear to be a git repository", remote_name); return; }
    };

    let url = match libgit::remote::GitUrl::parse(&remote.url) {
        Some(u) => u,
        None => { anyos_std::println!("fatal: invalid URL: {}", remote.url); return; }
    };
    ...
    for rref in &remote_refs {
        if rref.name.starts_with("refs/heads/") {
            let local_ref = rref.name.replace("refs/heads/", &format!("refs/remotes/{}/", remote_name));
...
fn cmd_pull(args: &anyos_std::args::ParsedArgs) {
    ...
    let remote_name = args.pos(1).unwrap_or("origin");
    let branch = repo.current_branch().ok().flatten().unwrap_or_else(|| String::from("main"));
    let remote_ref = format!("refs/remotes/{}/{}", remote_name, branch);

    let remote_oid = match libgit::refs::resolve_ref(&repo, &remote_ref) {
        Ok(oid) => oid,
        Err(_) => { anyos_std::println!("fatal: no tracking information for current branch"); return; }
    };
...
fn cmd_push(args: &anyos_std::args::ParsedArgs) {
    ...
    let branch = if args.pos_count >= 3 {
        ...
        repo.current_branch().ok().flatten().unwrap_or_else(|| String::from("main"))
    };

    let local_oid = match repo.head() {
        Ok(oid) => oid,
        Err(_) => { anyos_std::println!("fatal: no commits to push"); return; }
    };
    ...
    match libgit::transport::push_pack(&url, &updates, &pack_data) {
        Ok(response) => {
            if response.contains("unpack ok") || response.contains("000eunpack ok") {
                ...
                anyos_std::println!("   {}..{}  {} -> {}", old_oid.short(), local_oid.short(), branch, branch);
            } else {
...
fn cmd_tag(args: &anyos_std::args::ParsedArgs) {
    ...
    if args.pos_count < 2 {
        ...
        let oid = match repo.head() {
            Ok(o) => o,
            Err(_) => { anyos_std::println!("fatal: no commits"); return; }
        };
...
fn cmd_reset(args: &anyos_std::args::ParsedArgs) {
    ...
    let head_oid = match repo.head() {
        Ok(o) => o,
        Err(_) => { anyos_std::println!("fatal: no HEAD"); return; }
    };
    ...
    let commit = match Commit::parse(&commit_obj.data) {
        Some(c) => c,
        None => { anyos_std::println!("fatal: invalid commit"); return; }
    };
...
fn cmd_show(args: &anyos_std::args::ParsedArgs) {
    ...
    let oid = match oid {
        Some(o) => o,
        None => { anyos_std::println!("fatal: bad object {}", rev); return; }
    };
    ...
    match obj.obj_type {
        ObjectType::Commit => {
            let commit = match Commit::parse(&obj.data) { Some(c) => c, None => return };
            anyos_std::println!("\x1b[33mcommit {}\x1b[0m", oid.to_hex());
            anyos_std::println!("Author: {}", commit.author);
            anyos_std::println!();
            for line in commit.message.lines() {
        ...
        ObjectType::Tree => {
            ...
            for entry in &entries {
                anyos_std::println!("{:06o} {} {}\t{}", entry.mode, if entry.is_tree() { "tree" } else { "blob" }, entry.oid.short(), entry.name);
            }
        }
        ObjectType::Tag => {
            if let Ok(text) = core::str::from_utf8(&obj.data) {
...
fn get_head_tree_flat(repo: &Repository) -> Vec<(String, Oid)> {
    let head_oid = match repo.head() { Ok(o) => o, Err(_) => return Vec::new() };
    let commit_obj = match repo.read_object(&head_oid) { Ok(o) => o, Err(_) => return Vec::new() };
    let commit = match Commit::parse(&commit_obj.data) { Some(c) => c, None => return Vec::new() };
    let tree_obj = match repo.read_object(&commit.tree) { Ok(o) => o, Err(_) => return Vec::new() };
    parse_tree(&tree_obj.data).into_iter().map(|e| (e.name, e.oid)).collect()
}
...
fn get_user_info(repo: &Repository) -> (String, String) {
    ...
    let name = config.as_ref().and_then(|c| c.user_name())
        .or_else(|| global_config.as_ref().and_then(|c| c.user_name()))
        .unwrap_or("Unknown");
    let email = config.as_ref().and_then(|c| c.user_email())
...
fn month_days(month: u64) -> u64 {
    ...
    if month < 13 { C[month as usize] } else { 0 }
}
