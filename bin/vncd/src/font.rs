...
/// Number of glyphs in the font (printable ASCII 0x20–0x7F), 96 glyphs total.
...
/// The font data: 96 glyphs × 8 rows. Index with `(char - 0x20) * 8`.
#[rustfmt::skip]
pub static FONT8X8: [u8; GLYPH_COUNT * GLYPH_H] = [
    // 0x20 SPACE
    ...
    // 0x7F DEL (solid block, used as fallback/replacement glyph)
    0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
];

/// Draw a single character at pixel position `(px, py)` into an ARGB framebuffer.
///
/// - `fb`     — mutable slice of ARGB pixels (width × height)
/// - `stride` — row stride in pixels (typically framebuffer width)
/// - `ch`     — ASCII character to draw
/// - `px`,`py`— top-left pixel coordinates
...
pub fn draw_char(
    ...
    let glyph = &FONT8X8[glyph_idx * GLYPH_H..(glyph_idx + 1) * GLYPH_H];

    let bg_alpha = (bg >> 24) & 0xFF;
...
/// Draw an ASCII string starting at `(px, py)`.
///
/// Characters are placed left-to-right with no word-wrap.
/// Returns the x coordinate immediately after the last character drawn.
pub fn draw_str(
    fb: &mut [u32],
    stride: usize,
    s: &[u8],
    ...
    let mut x = px;
    for &ch in s {
        draw_char(fb, stride, ch, x, py, fg, bg);
        x += GLYPH_W;
    }
    x
}
