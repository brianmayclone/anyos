...
use anyos_std::fmt::{fmt_pct, fmt_u32};
...
pub fn print_mem_bar(used_kb: u32, total_kb: u32, total_width: usize) {
    let pct = if total_kb > 0 {
        (used_kb as u64 * 100 / total_kb as u64) as u32
    } else {
        0
    };
    let mut sa = [0u8; 12];
    ...
    let fill_area = if total_width > overhead {
...
fn render_task_row(
    ...
    let net_color = if task.net_kbit > 1000 {
        FG_BGREEN
    } else {
        RESET
    };
    ...
    let cpu_color = if task.cpu_pct_x10 > 500 {
        FG_BRED
    } else {
        RESET
    };
    ...
    let name_trunc = if name.len() > cmd_w {
        &name[..cmd_w]
    } else {
        name
    };
