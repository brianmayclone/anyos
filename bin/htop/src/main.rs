...
fn main() {
    ...
    static mut RAW_BUF: [u8; THREAD_ENTRY_SIZE * MAX_TASKS] = [0u8; THREAD_ENTRY_SIZE * MAX_TASKS];
    ...
    loop {
        ...
        elapsed_ms += 50;
        if elapsed_ms < REFRESH_MS && !need_redraw {
            continue;
        }
        elapsed_ms = 0;
        need_redraw = false;
        ...
        fetch_cpu(&mut cpu);
        let task_count =
            unsafe { fetch_tasks(&mut RAW_BUF, &mut prev, cpu.total_sched_ticks, &mut tasks) };
