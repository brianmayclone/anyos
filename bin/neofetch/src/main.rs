...
fn main() {
    ...
    let threads = if thread_ret != u32::MAX {
        thread_ret
    } else {
        0
    };
    ...
    // Uptime
    {
        ...
        if days > 0 {
            ...
            if days == 1 {
                l.push(b" ");
                l.push(t("day").as_bytes());
                l.push(b", ");
            } else {
                l.push(b" ");
                l.push(t("days").as_bytes());
                l.push(b", ");
            }
        }
        if hours > 0 || days > 0 {
            ...
            if hours == 1 {
                l.push(b" ");
                l.push(t("hour").as_bytes());
                l.push(b", ");
            } else {
                l.push(b" ");
                l.push(t("hours").as_bytes());
                l.push(b", ");
            }
        }
        l.push_u32(mins);
        if mins == 1 {
            l.push(b" ");
            l.push(t("min").as_bytes());
        } else {
            l.push(b" ");
            l.push(t("mins").as_bytes());
        }
        lc += 1;
    }
    ...
    // Shell
    {
        let l = &mut lines[lc];
        l.push(BOLD_CYAN);
        l.push(t("Shell").as_bytes());
        l.push(RESET);
        l.push(b": ");
        if shell.is_empty() {
            l.push(t("unknown").as_bytes());
        } else {
            l.push(shell);
        }
        lc += 1;
    ...
    if has_net && (ip[0] != 0 || ip[1] != 0 || ip[2] != 0 || ip[3] != 0) {
        let l = &mut lines[lc];
        l.push(BOLD_CYAN);
        l.push(t("Local IP").as_bytes());
        l.push(RESET);
        l.push(b": ");
        l.push_u32(ip[0] as u32);
        l.push(b".");
        l.push_u32(ip[1] as u32);
        l.push(b".");
        l.push_u32(ip[2] as u32);
        l.push(b".");
