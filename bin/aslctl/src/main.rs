...
fn parse_command<'a>(args: &anyos_std::args::ParsedArgs<'a>) -> Option<ClientCommand<'a>> {
    let cmd = args.pos(0)?;
    match cmd {
...
fn parse_response(raw: &str) -> Result<WireResponse, &'static str> {
...
fn print_response(command: &ClientCommand<'_>, response: &WireResponse) {
    match response {
        ...
        WireResponse::Ok { count, lines } => match command {
            ...
            ClientCommand::MountList(_) | ClientCommand::MountShow { .. } | ClientCommand::MountAdd { .. } | ClientCommand::MountRemove { .. } => {
...
mod tests {
    ...
    fn parses_port_add_command() {
        let args = anyos_std::args::parse(
            "port add ubuntu-dev web 127.0.0.1 3000 3000 tcp Web",
            b"",
        );
        match parse_command(&args) {
