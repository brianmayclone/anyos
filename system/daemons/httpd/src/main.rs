...
const HTTPD_DIRS: &[&str] = &["config", "sites", "sites/default"];
const HTTPD_DEFAULTS: &[libconf_schema::DefaultEntry<'static>] = &[
    default_string("config/default_index_csv", "index.html,index.htm"),
    default_bool("config/log", true),
    default_string("sites/default/name", "Default Web Site"),
    default_int("sites/default/port", 80),
    default_string("sites/default/root", "/Users/Shared/www"),
    default_string("sites/default/index_csv", "index.html,index.htm"),
    default_bool("sites/default/enabled", true),
    default_string("sites/default/rewrites_blob", ""),
];
const HTTPD_MANIFEST: libconf_schema::RegistryManifest<'static> = manifest(
    "services/httpd",
...
const HTTPD_SCHEMA: ServiceSchema<'static> = ServiceSchema::new("httpd", &HTTPD_MANIFEST);
...
fn conf_u16(client: &mut ConfClient, path: &str) -> Option<u16> {
    match client.get(RegistryScope::System, path).ok()?.value {
        Some(ConfValue::Int(value)) if (0..=u16::MAX as i64).contains(&value) => Some(value as u16),
        _ => None,
    }
}
...
fn parse_global_config() -> GlobalConfig {
    ...
    register_manifest();
    if let Ok(mut client) = ConfClient::connect("httpd") {
        if let Some(value) = conf_string(&mut client, "services/httpd/config/default_index_csv") {
            let parsed = split_csv(&value);
            if !parsed.is_empty() {
                cfg.default_index = parsed;
            }
        }
        if let Some(value) = conf_bool(&mut client, "services/httpd/config/log") {
            cfg.log = value;
        }
    }
    cfg
...
fn load_site_from_registry(
    client: &mut ConfClient,
    site_key: &str,
    default_index: &[String],
) -> Option<SiteConfig> {
    let base = format!("services/httpd/sites/{}", site_key);
    let name = conf_string(client, &format!("{}/name", base))?;
    let root = conf_string(client, &format!("{}/root", base))?;
    let port = conf_u16(client, &format!("{}/port", base)).unwrap_or(80);
    let enabled = conf_bool(client, &format!("{}/enabled", base)).unwrap_or(true);
    let mut index_files = conf_string(client, &format!("{}/index_csv", base))
        .map(|value| split_csv(&value))
        .unwrap_or_default();
    if index_files.is_empty() {
        index_files = default_index.to_vec();
    }

    let rewrites = conf_string(client, &format!("{}/rewrites_blob", base))
        .map(|value| {
            let mut items = Vec::new();
            for line in value.split('\n') {
                if let Some(rule) = parse_rewrite_rule(line.trim()) {
                    items.push(rule);
                }
            }
            items
...
fn load_all_sites(default_index: &[String]) -> Vec<SiteConfig> {
    ...
    register_manifest();
    let Ok(mut client) = ConfClient::connect("httpd") else {
        return sites;
    };
    let Ok(items) = client.list(RegistryScope::System, "services/httpd/sites") else {
        return sites;
    };
    ...
    for item in &items {
        ...
        let Some(rest) = item.path.strip_prefix("services/httpd/sites/") else {
            continue;
        };
        if rest.is_empty() || rest.contains('/') {
            continue;
        }
        if !site_keys.iter().any(|entry: &String| entry == rest) {
            site_keys.push(String::from(rest));
        }
    }

    for site_key in &site_keys {
        if let Some(site) = load_site_from_registry(&mut client, site_key, default_index) {
            if site.enabled {
                sites.push(site);
            }
        }
...
fn main() {
    ...
    if args.contains("--help") || args.contains("-h") {
        ...
        println!("");
        println!("Config namespace: services/httpd");
        println!("Manage: svc start httpd / svc stop httpd");
        return;
    }

    let mut lifecycle = ServiceLifecycle::connect("httpd").ok();
    ...
    if sites.is_empty() {
        println!("httpd: no enabled sites found in services/httpd/sites");
        println!("httpd: register or configure a site and try again");
        if let Some(lifecycle) = lifecycle.as_mut() {
            let _ = lifecycle.notify_failed("no_enabled_sites");
        }
        return;
    }
    ...
    if pipe_id == 0 || pipe_id == u32::MAX {
        println!("httpd: failed to create control pipe");
        if let Some(lifecycle) = lifecycle.as_mut() {
