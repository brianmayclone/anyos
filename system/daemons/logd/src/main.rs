...
const LOGD_DIRS: &[&str] = &["config"];
const LOGD_DEFAULTS: &[libconf_schema::DefaultEntry<'static>] = &[
    default_string("config/log_dir", DEFAULT_LOG_DIR),
    default_int("config/max_size", DEFAULT_MAX_SIZE as i64),
    default_int("config/max_files", DEFAULT_MAX_FILES as i64),
    default_bool("config/kernel", true),
    default_int("config/flush_interval", DEFAULT_FLUSH_INTERVAL as i64),
];
const LOGD_MIGRATIONS: &[libconf_schema::MigrationStep<'static>] = &[];
const LOGD_MANIFEST: libconf_schema::RegistryManifest<'static> = manifest(
    "services/logd",
...
const LOGD_SCHEMA: ServiceSchema<'static> = ServiceSchema::new("logd", &LOGD_MANIFEST);
...
impl Config {
    /// Load configuration from confd, falling back to defaults.
    fn load() -> Self {
        let cfg = Config {
        ...
        cfg.with_registry_overrides()
    }

    fn with_registry_overrides(mut self) -> Self {
        if let Some(v) = LOGD_SCHEMA.read_i64("config/max_size") {
            if v > 0 && v <= u32::MAX as i64 {
                self.max_size = v as u32;
            }
        }
        if let Some(v) = LOGD_SCHEMA.read_i64("config/max_files") {
            if v > 0 && v <= u32::MAX as i64 {
                self.max_files = v as u32;
            }
        }
        if let Some(v) = LOGD_SCHEMA.read_bool("config/kernel") {
            self.kernel = v;
        }
        if let Some(v) = LOGD_SCHEMA.read_i64("config/flush_interval") {
            if v > 0 && v <= u32::MAX as i64 {
                self.flush_interval = v as u32;
            }
        }
        self
    }
...
fn main() {
    ...
    let _ = LOGD_SCHEMA.register();
    let cfg = Config::load();
    let mut lifecycle = ServiceLifecycle::connect("logd").ok();
