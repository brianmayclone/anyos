...
fn main() {
    ...
    if raw.contains("--help") {
        anyos_std::println!("vncd - VNC server daemon\n\nUsage: vncd");
        return;
    }

    let mut lifecycle = ServiceLifecycle::connect("vncd").ok();
    ...
    if let Some(lifecycle) = lifecycle.as_mut() {
        ...
        let _ = lifecycle.set_health(if cfg.enabled { "ready" } else { "disabled" });
    }
    loop {
        // ── Poll control pipe ─────────────────────────────────────────────
