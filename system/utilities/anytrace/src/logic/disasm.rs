...
pub fn decode(code: &[u8], rip: u64) -> Option<DecodedInstr> {
    ...
    match opcode {
        ...
        0x0F => {
            ...
            match op2 {
                ...
                0xB6 => {
                    ...
                    if pos < code.len() {
                        let (ops, consumed) =
                            decode_modrm_reg(code, pos, rex_r, rex_b, rex_w, true, false);
                        pos += consumed;
        ...
        0x89 => {
            ...
            if pos < code.len() {
                let (ops, consumed) =
                    decode_modrm_reg(code, pos, rex_r, rex_b, rex_w, false, false);
                pos += consumed;
                instr.set_operands(&ops);
            }
        }
...
pub fn decode_block(
    ...
    base_rip: u64,
    max_instrs: usize,
) -> alloc::vec::Vec<(u64, DecodedInstr)> {
...
fn decode_modrm_reg(
    ...
    let reg_name = if reg_idx < regs.len() {
        regs[reg_idx]
    } else {
        "???"
    };
...
fn decode_modrm_rm(code: &[u8], start: usize, rex_b: bool, rex_w: bool) -> (String, usize) {
    ...
    if md == 3 {
        // Register direct
        let name = if (rm as usize) < regs.len() {
            regs[rm as usize]
        } else {
            "???"
        };
    ...
    if (rm & 7) == 4 {
        ...
        if md == 0 && (base & 7) == 5 {
            ...
            base_reg = String::from(if (base as usize) < REG64.len() {
                REG64[base as usize]
            } else {
                "???"
            });
        ...
        if index != 4 {
            let idx_name = if (index as usize) < REG64.len() {
        ...
        let disp = i32::from_le_bytes([
            code[start + consumed],
            code[start + consumed + 1],
            code[start + consumed + 2],
            code[start + consumed + 3],
        ]);
        consumed += 4;
        return (alloc::format!("[rip{:+#x}]", disp), consumed);
    } else {
        base_reg = String::from(if (rm as usize) < REG64.len() {
            REG64[rm as usize]
        } else {
            "???"
        });
