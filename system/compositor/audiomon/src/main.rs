...
use libcompositor_client::{
    TrayClient, WindowHandle, EVT_FOCUS_LOST, EVT_MOUSE_DOWN, EVT_MOUSE_MOVE, EVT_MOUSE_UP,
    EVT_STATUS_ICON_CLICK, EVT_WINDOW_CLOSE,
};
...
fn draw_speaker_icon(pixels: &mut [u32; 256], volume: u8, muted: bool, available: bool) {
    ...
    if !available {
        for i in 0..6 {
            ...
            let y2 = 12 - i;
            if x < 16 && y1 < 16 {
                pixels[y1 * 16 + x] = 0xFFFF3B30;
            }
            if x < 16 && y2 < 16 {
                pixels[y2 * 16 + x] = 0xFFFF3B30;
            }
        }
    } else if muted {
        for i in 0..4 {
            ...
            let y2 = 10 - i;
            if x < 16 {
                pixels[y1 * 16 + x] = color;
            }
            if x < 16 {
                pixels[y2 * 16 + x] = color;
            }
        }
    } else {
        if volume > 0 {
...
impl PopupState {
    ...
    fn hit_slider(&self, mx: i32, my: i32) -> bool {
        mx >= self.slider_x
            && mx < self.slider_x + self.slider_w as i32
            && my >= self.slider_y - 4
            && my < self.slider_y + 24
    }
...
fn fmt_volume<'a>(buf: &'a mut [u8; 8], vol: u32) -> &'a str {
    ...
    if vol >= 100 {
        ...
        buf[pos] = b'0' + (vol / 10) as u8;
        pos += 1;
        buf[pos] = b'0' + (vol % 10) as u8;
        pos += 1;
    } else {
        buf[pos] = b'0' + vol as u8;
        pos += 1;
    }
    buf[pos] = b'%';
    pos += 1;
...
fn main() {
    ...
    let mut volume = if available {
        anyos_std::audio::audio_get_volume()
    } else {
        0
    };
            ...
            match event.event_type {
                        ...
                        if event.window_id == win.id {
                            if let Some(ref mut state) = popup_state {
                                ...
                                if state.hit_slider(mx, my) {
                                    ...
                                    if state.muted {
                                        state.muted = false;
                                        volume = if state.saved_volume > 0 {
                                            state.saved_volume
                                        } else {
                                            50
                                        };
                ...
                EVT_MOUSE_MOVE => {
                    if let Some(ref win) = popup {
                        if event.window_id == win.id {
                            if let Some(ref mut state) = popup_state {
                                if !available || !state.slider_dragging {
                                    continue;
                                }
                                let mx = event.arg1 as i32;
                                let new_val = state.slider_value_from_x(mx);
