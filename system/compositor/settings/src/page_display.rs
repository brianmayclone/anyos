...
pub fn build(parent: &ui::ScrollView) -> u32 {
    ...
    // (Display scale is now a per-output DropDown inside the
    // multi-monitor section; the standalone DPI card was removed
    // because displayd mirrors the primary output's scale_percent
    // into UISYS_SCALE on every layout commit.)
