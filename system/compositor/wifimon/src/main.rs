...
const WIFIMON_DIRS: &[&str] = &["config"];
const WIFIMON_DEFAULTS: &[libconf_schema::DefaultEntry<'static>] =
    &[default_string("config/known_networks_blob", "")];
const WIFIMON_MIGRATIONS: &[libconf_schema::MigrationStep<'static>] = &[];
const WIFIMON_MANIFEST: libconf_schema::RegistryManifest<'static> = manifest(
    "profile/wifi",
    RegistryScope::User,
    1,
    WIFIMON_DIRS,
...
const WIFIMON_SCHEMA: ServiceSchema<'static> = ServiceSchema::new("wifimon", &WIFIMON_MANIFEST);

fn load_known_networks() -> Vec<(Vec<u8>, Vec<u8>)> {
    let mut result = Vec::new();
    if let Some(blob) = WIFIMON_SCHEMA.read_string("config/known_networks_blob") {
        parse_known_networks_blob(&blob, &mut result);
    }
    result
}
...
fn save_known_networks(networks: &[(Vec<u8>, Vec<u8>)]) {
    ...
    if let Ok(text) = core::str::from_utf8(&data) {
        let _ = WIFIMON_SCHEMA.write_string("config/known_networks_blob", text);
    }
}

fn parse_known_networks_blob(blob: &str, result: &mut Vec<(Vec<u8>, Vec<u8>)>) {
    for line in blob.as_bytes().split(|&b| b == b'\n') {
