...
const INPUTMON_SCHEMA: ServiceSchema<'static> = ServiceSchema::new("inputmon", &INPUTMON_MANIFEST);
...
fn load_config() {
    if let Some(id) = INPUTMON_SCHEMA.read_i64("config/layout") {
        anyos_std::kbd::set_layout(id as u32);
    }
}

fn save_config(id: u32) {
    let _ = INPUTMON_SCHEMA.write_i64("config/layout", id as i64);
}
