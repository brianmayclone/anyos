...
static void ExtendLumpInfo(int newnumlumps)
{
    ...
    printf("[DBG] ExtendLumpInfo: calloc(%d, %d) = %p .. %p (old lumpinfo=%p numlumps=%d)\n",
           newnumlumps, (int)sizeof(lumpinfo_t), (void*)newlumpinfo,
           (void*)((char*)newlumpinfo + newnumlumps * sizeof(lumpinfo_t)),
           (void*)lumpinfo, numlumps);
...
wad_file_t *W_AddFile (char *filename)
{
    ...
    printf("[DBG] W_AddFile: filling lumps %d..%d, wad_file=%p file_class=%p\n",
           startlump, (int)numlumps-1, (void*)wad_file,
           (void*)(wad_file ? wad_file->file_class : 0));
    ...
    /* Verify first and last entry after filling */
    printf("[DBG] W_AddFile: verify lumpinfo[%d].wad_file=%p, lumpinfo[%d].wad_file=%p\n",
           startlump, (void*)lumpinfo[startlump].wad_file,
           (int)numlumps-1, (void*)lumpinfo[numlumps-1].wad_file);
...
void W_ReadLump(unsigned int lump, void *dest)
{
    ...
    if (l->wad_file == NULL || l->wad_file->file_class == NULL) {
        char name8[9]; memcpy(name8, l->name, 8); name8[8] = 0;
        printf("[DBG] W_ReadLump: BAD lump %d '%s' wad_file=%p file_class=%p\n",
               lump, name8, (void*)l->wad_file,
               l->wad_file ? (void*)l->wad_file->file_class : (void*)0);
        printf("[DBG]   lumpinfo=%p &lumpinfo[%d]=%p\n",
               (void*)lumpinfo, lump, (void*)l);
    }
...
void *W_CacheLumpNum(int lumpnum, int tag)
{
    ...
    if (lump->wad_file == NULL || lump->wad_file->file_class == NULL) {
        char name8[9]; memcpy(name8, lump->name, 8); name8[8] = 0;
        printf("[DBG] W_CacheLumpNum: BAD lump %d '%s' wad_file=%p file_class=%p\n",
               lumpnum, name8, (void*)lump->wad_file,
               lump->wad_file ? (void*)lump->wad_file->file_class : (void*)0);
        printf("[DBG]   lumpinfo=%p &lumpinfo[%d]=%p\n",
               (void*)lumpinfo, lumpnum, (void*)lump);
    }
