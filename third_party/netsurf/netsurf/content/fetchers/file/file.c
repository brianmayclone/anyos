...
fetch_file_process_aborted:
	...
	if (infile == NULL) {
		fprintf(stderr, "[file-fetch] fopen FAILED: %s\n", ctx->path);
		/* process errors as appropriate */
		fetch_file_process_error(ctx,
				fetch_file_errno_to_http_code(errno));
		return;
	...
	fprintf(stderr, "[file-fetch] fread path: %s size=%zu\n", ctx->path, buf_size);

	/* allocate the buffer storage */
	buf = malloc(buf_size);
	if (buf == NULL) {
		fprintf(stderr, "[file-fetch] malloc(%zu) FAILED\n", buf_size);
		msg.type = FETCH_ERROR;
		msg.data.error =
	...
	if (ctx->aborted == false) {
		fprintf(stderr, "[file-fetch] DONE: %s read=%zd bytes\n", ctx->path, tot_read);
		msg.type = FETCH_FINISHED;
		fetch_file_send_callback(&msg, ctx);
	}
