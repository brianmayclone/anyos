...
static void framebuffer_run(void)
{
	...
	fprintf(stderr, "[browser] framebuffer_run() entered\n");
	...
	while (fb_complete != true) {
		...
		loop_count++;
		if (loop_count <= 10 || (loop_count % 500) == 0)
			fprintf(stderr, "[browser] loop #%d timeout=%d redraw_pending=%d\n",
				loop_count, timeout, fbtk_get_redraw_pending(fbtk));
	...
	fprintf(stderr, "[browser] framebuffer_run() exiting\n");
}
		...
		gui_window_create_flags flags)
{
	...
	fprintf(stderr, "[browser] gui_window_create called\n");

	gw = calloc(1, sizeof(struct gui_window));

	if (gw == NULL) {
		fprintf(stderr, "[browser] gui_window_create: calloc FAILED\n");
		return NULL;
	}
	...
	fprintf(stderr, "[browser] gui_window_create: window=%p\n", (void *)gw->window);
	...
	fprintf(stderr, "[browser] gui_window_create: done, redraw_pending=%d\n",
		fbtk_get_redraw_pending(fbtk));
...
main(int argc, char** argv)
{
	...
	struct netsurf_table framebuffer_table = {
        ...
        fprintf(stderr, "[browser] main() starting\n");
        ret = netsurf_register(&framebuffer_table);
        if (ret != NSERROR_OK) {
		die("NetSurf operation table failed registration");
        }
	fprintf(stderr, "[browser] netsurf_register OK\n");

	respaths = fb_init_resource_path(NETSURF_FB_RESPATH":"NETSURF_FB_FONTPATH);
	fprintf(stderr, "[browser] resource path: " NETSURF_FB_RESPATH "\n");
	...
	fprintf(stderr, "[browser] nsoption_init OK\n");
	options = filepath_find(respaths, "Choices");
	fprintf(stderr, "[browser] Choices path: %s\n", options ? options : "(null)");
	...
	fprintf(stderr, "[browser] Messages path: %s\n", messages ? messages : "(null)");
        ret = messages_add_from_file(messages);
	free(messages);
	if (ret != NSERROR_OK) {
		fprintf(stderr, "[browser] Message translations failed to load (ret=%d)\n", ret);
	}

	/* common initialisation */
	fprintf(stderr, "[browser] calling netsurf_init...\n");
	ret = netsurf_init(NULL);
	if (ret != NSERROR_OK) {
		die("NetSurf failed to initialise");
	}
	fprintf(stderr, "[browser] netsurf_init OK\n");
	...
	fprintf(stderr, "[browser] cmdline OK, fename=%s feurl=%s %dx%d\n",
		fename ? fename : "(null)", feurl ? feurl : "(null)", fewidth, feheight);

	nsfb = framebuffer_initialise(fename, fewidth, feheight, febpp);
	if (nsfb == NULL)
		die("Unable to initialise framebuffer");
	fprintf(stderr, "[browser] framebuffer_initialise OK\n");

	framebuffer_set_cursor(&pointer_image);

	if (fb_font_init() == false)
		die("Unable to initialise the font system");
	fprintf(stderr, "[browser] fb_font_init OK\n");

	fbtk = fbtk_init(nsfb);
	fprintf(stderr, "[browser] fbtk_init OK\n");
	...
	fprintf(stderr, "[browser] calling browser_window_create for '%s'\n",
		feurl ? feurl : "(null)");
	...
	fprintf(stderr, "[browser] browser_window_create ret=%d\n", ret);
	if (ret != NSERROR_OK) {
